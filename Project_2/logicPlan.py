...
def atMostOne(literals: List[Expr]) -> Expr:
    ...
    """
    # create a list containing all conjunctions
    conjunctions = list()
    nol = len(literals)
    
    for i in range(0, nol):
        for j in range(i+1, nol): 
            conjunctions.append(disjoin(~literals[i], ~literals[j]))
...
def exactlyOne(literals: List[Expr]) -> Expr:
    ...
    the expressions in the list is true.
    """
    # exactly one: basically at most once & at least once 
    at_least_once = atLeastOne(literals)
    at_most_once = atMostOne(literals)
    return conjoin(at_least_once, at_most_once)
...
def pacphysicsAxioms(t: int, all_coords: List[Tuple], non_outer_wall_coords: List[Tuple], walls_grid: List[List] = None,
    ...
    wall_list = [logic.PropSymbolExpr(pacman_str, wall_coordinate[0], wall_coordinate[1], time=t)
                 for wall_coordinate in non_outer_wall_coords]
...
def foodLogicPlan(problem) -> List:
    ...
    for t in range(50):
        ...
        for each_food_coord in food:
            x, y = each_food_coord
            pacman_location = PropSymbolExpr(pacman_str, x, y, time=t)
            this_food_location = PropSymbolExpr(food_str, x, y, time=t)
            next_food_location = PropSymbolExpr(food_str, x, y, time=t + 1)
...
def slam(problem, agent) -> Generator:
    '''
    problem: a SLAMProblem instance # Đề bài Slam
    agent: a SLAMLogicAgent instance # Logic của Slam
    '''
    pac_x_0, pac_y_0 = problem.startState # Vị trí bắt đầu của Slam
    KB = [] # Knowledge Base: nơi chứa các biểu thức liên quan
    all_coords = list(itertools.product(range(problem.getWidth()+2), range(problem.getHeight()+2)))
    # all_coords: Đại diện cho danh sách tất cả cá tọa độ trong môi trường, bao gồm cả viên bản đồ
    non_outer_wall_coords = list(itertools.product(range(1, problem.getWidth()+1), range(1, problem.getHeight()+1)))
    # none_outer_wall_coords: danh sách chứa các tọa độ của các ô không nằm trên viền bản đồ
    # map describes what we know, for GUI rendering purposes. -1 is unknown, 0 is open, 1 is wall
    known_map = [[-1 for y in range(problem.getHeight()+2)] for x in range(problem.getWidth()+2)]
    #known_map: đại diện cho bản đồ, là ma trận 2 chiều với giá trị khởi tạo -1 cho tất cả các ô
    # Giá trị -1: Đại diện cho ô chưa biết
    # Giá trị 0: Đại diện cho ô trống hoặc đường đi, ô này được xác định không phải là tường
    # Giá trị 1: Đại diện cho ô là tường.
    # We know that the outer_coords are all walls.
    outer_wall_sent = []
    # Vòng lặp từng cặp tọa dộ trong all_coords
    for x, y in all_coords:
        # Kiểm tra xem tọa độ x, y có nằm trên viền bản đồ hay không
        if ((x == 0 or x == problem.getWidth() + 1)
                or (y == 0 or y == problem.getHeight() + 1)):
            # Nếu có gán giá trị bằng 1 => tức là tường
            known_map[x][y] = 1
            # Thêm phần tử vào danh sách outer_wall_sent thông qua biểu thức PropSymbolExpr và kết hợp(conjoin) thành biểu thức logic
            outer_wall_sent.append(PropSymbolExpr(wall_str, x, y))
    KB.append(conjoin(outer_wall_sent))
    # Thêm vào KB
    ...
    "*** BEGIN YOUR CODE HERE ***"
    # Lấy vị trí ban đầu của Pacman và thêm vào bộ tri thức KB
    KB.append(logic.PropSymbolExpr(pacman_str, pac_x_0, pac_y_0, time=0))
    # tạo biểu thức logic PropSymbolExpr với tên là pacman_str
    # tọa độ (pac_x_0, pac_y_0)
    # time = 0: biểu thức logi áp dụng cho thời điểm ban đầu
    # Cập nhập known_map dựa trên vị trí ban đầu
    known_map[pac_x_0][pac_y_0]=0

    # Thêm biểu thức logic thích hợp vào bộ tri thức
    # Ý nghĩa: Phản ánh ô tại vị trí ban đầu không phải là tường
    KB.append(~logic.PropSymbolExpr(wall_str, pac_x_0, pac_y_0))


    # Lặp qua các bước thời gian từ 0 tới agent.num_timesteps
    ...
    for t in range(agent.num_timesteps):
        ...
        for wall in non_outer_wall_coords:
            # Tìm các vị trí tường không phải là viền trong bộ tri thức KB
            find_wall(KB, wall, known_map)

            # Tìm các vị trí mà pacman có thể đi tới
            find_pac_location(KB, t, wall, possibles)

        # Di chuyển Pacman tới trạng thái tiếp theo dựa trên hành động của Pacman tại thời điểm t
        agent.moveToNextState(agent.actions[t])
        "*** END YOUR CODE HERE ***"
        yield (known_map, possibles)
